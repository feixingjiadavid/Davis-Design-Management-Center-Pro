...
import { archiveStorageObject } from "./drive-archive.ts";
...
Deno.serve(async(req:Request)=>{
  ...
  // 故意保持 ready_for_demo，避免旧工作台触发整页轮询重绘；真实生成态由 v3 Demo 控制器读取 generation rows 展示。
  ...
  try{
    ...
    let drive:any={archived:false,status:'storage_only'};
    if(generated?.storage_path){try{drive=await archiveStorageObject(admin,{taskId,pageIndex,storagePath:String(generated.storage_path)});}catch(error){drive={archived:false,status:'failed',error:error instanceof Error?error.message:String(error)};}}
    const output={...generated,...drive,run_id:runId||null,page_index:pageIndex,page_count:pages.length,page_title:page.title,exact_copy:page.copy,prompt_version:DEMO_PROMPT_VERSION,style_reference_count:refs.length,design_asset_count:required.length,model_input_count:modelInputs.length,persistent_storage:'supabase_storage'};
    const updated=await admin.from('uat_design_generations').update({status:'ready',output,updated_at:new Date().toISOString()}).eq('id',queued.data.id).select('*').single();if(updated.error)throw updated.error;
    ...
    await admin.from('test_tasks').update({status:complete?'demo_review':'ready_for_demo',summary_desc:complete?`Seedream 4.0 Demo 已完成 ${pages.length}/${pages.length}，等待确认`:`P${pageIndex} 已完成并持久化；下一次继续 P${Math.min(pages.length,pageIndex+1)}`}).eq('id',taskId);
    await admin.from('uat_audit_log').insert({actor_id:user.id,actor_email:user.email,action:'demo_page_generated',task_id:taskId,details:{generation_id:updated.data.id,page_index:pageIndex,page_count:pages.length,run_id:runId,prompt_version:DEMO_PROMPT_VERSION,drive_archive_status:drive.status}});
    return out({ok:true,status:'ready',generation:updated.data,complete,drive_archive:drive});
  }catch(error){
