...
export const DEMO_PROMPT_BUILD = "scene-directed-20260818-v21";
...
export function buildCreativeDemoPrompt(args:{brief:Record<string,any>;page:Page;styleReference:StyleReference;assets:Asset[]}){
  ...
  const assetText=assets.slice(0,9).map((a,i)=>`图${i+2}：${a.asset_role||a.file_name||'必用素材'}；${a.note||'保持原始身份与外观'}`).join("\n");
...
- WeBank Logo是品牌签名，不是主视觉。浅色区域用彩色版，深色区域用反白版；禁止重绘、变色、变形或制造新的Logo。
- IP必须参与场景：指向榜单、站上领奖台、沿任务路径行动、与标题或道具穿插均可；禁止孤零零站在页面底部。

【材质与排版】
统一使用复古拼贴的纸张、撕边、网点、颗粒、胶带、印章、阴影等材质连接全页。字体要有设计感但清楚可读；标题可以描边、错位、透视、纸片叠压，正文保持阅读效率。四周留安全边距，不裁切核心标题、金额、IP或Logo。

【硬性限制】
${compactList(brief.constraints,8)||'尺寸与品牌元素必须准确'}

【输出前内部自检】
如果画面仍然可以概括为“色块 + 文案 + IP + Logo”，说明设计没有完成，必须推翻并重新组织成连续场景；只有达到品牌活动主视觉的叙事、层次和材质统一后再输出。

只输出当前这一页完整竖版设计，不输出线框、解释、草稿或设计说明。`;
