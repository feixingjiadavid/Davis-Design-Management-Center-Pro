...
import { generateSeedreamDemo, SEEDREAM_DEMO_PROXY_URL } from "./seedream-client.ts";

test("Seedream Demo client forwards UAT JWT, dimensions, page context and image inputs to proxy", async () => {
  let capturedUrl = "";
  ...
  const fetcher: typeof fetch = async (url: any, init?: RequestInit) => {
    ...
    return new Response(JSON.stringify({
      ...
      requested_size: "1242x1660",
      actual_size: "1242x1660",
  ...
  assert.equal(body.width, 1242);
  assert.equal(body.height, 1660);
