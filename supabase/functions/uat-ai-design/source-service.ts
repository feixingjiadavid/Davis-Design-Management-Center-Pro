...
async function findOrCreateSource(
  admin: any,
  taskId: string,
  sourceType: "form_fields" | "tencent_doc",
  ...
  const inserted = await admin.from("uat_requirement_sources").insert({
    task_id: taskId,
...
async function persistReadResult(admin: any, source: any, result: SourceReadResult) {
  if (result.status !== "ready") {
    await admin.from("uat_requirement_sources").update({
  ...
  const existing = (await admin.from("uat_source_snapshots")
    .select("id")
    .eq("source_id", source.id)
    .eq("content_sha256", document.contentSha256)
    .maybeSingle()).data;
  ...
  if (!snapshotId) {
    ...
    await admin.from("uat_requirement_analyses").update({ status: "stale", updated_at: new Date().toISOString() })
      .eq("task_id", source.task_id)
      .in("status", ["clarification_required", "understanding_ready", "confirmed"]);
  }
  await admin.from("uat_requirement_sources").update({
    status: "ready",
...
export async function ingestTaskSources(
  ...
  if (task.link?.trim()) {
    ...
    const linkResult = await readTencentPublic(sourceUrl, fetcher);
