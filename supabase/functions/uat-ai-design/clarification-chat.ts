...
export async function saveRequesterAnswers(admin: any, taskId: string, answersInput: unknown, message: string, clientRequestId: string, userId: string) {
  ...
  for (const answer of answers) {
    ...
    await admin.from("uat_clarification_messages").insert({ task_id: taskId, clarification_id: row.id, sender_id: userId, sender_role: "requester", message_type: "answer", content: answer.answer, metadata: { question: row.question } });
  }
