import { activeRevision, latestRequesterFeedback, parseHistory, revisionStage } from './revision-cycle-core.mjs?v=revision-loop-v1';
...
async function load() {
  ...
  return { task, template, revisions, history, latest:activeRevision(revisions), feedback:latestRequesterFeedback(history) };
}
...
function cycleHistoryHtml(revisions) {
  if (!revisions.length) return '';
  const items = [...revisions].sort((a,b)=>Number(a.revision_no||0)-Number(b.revision_no||0));
  return `<div class="mt-5 pt-5 border-t border-white/10"><div class="flex items-center justify-between"><p class="text-[11px] font-bold text-white">内容修改循环记录</p><span class="text-[10px] text-zinc-500">共 ${items.length} 轮</span></div><div class="mt-3 space-y-2">${items.map((revision) => {
...
function statusBlock() {
  const { task, latest, feedback } = state;
  if (!latest) {
    if (feedback) return `<div class="rounded-xl border border-blue-500/20 bg-blue-500/5 p-4"><p class="text-[13px] font-bold text-blue-300">AI 设计师已收到修改意见</p><p class="text-[11px] text-zinc-400 mt-2 leading-relaxed">${esc(feedback.feedback)}</p><p class="text-[10px] text-zinc-500 mt-2">AI 正在进行真实需求理解；需要修改图片时会由 AI 设计师执行。</p></div>`;
...
function syncHeader() {
  ...
  let text = '母版已锁定';
  if (state.latest?.status === 'ready_for_review') text = `第 ${state.latest.revision_no} 次修改待验收`;
  else if (['generation_requested','generating'].includes(String(state.latest?.status))) text = `第 ${state.latest.revision_no} 次修改生成中`;
  else if (state.feedback && !state.latest) text = '修改意见已交给 AI';
...
function keyOf(next) {
  return JSON.stringify({ status:next.task.status, summary:next.task.summary_desc, template:next.template?.id || '', revisions:next.revisions.map(r=>[r.id,r.revision_no,r.status,r.affected_pages,r.system_content,r.generated_at]), feedback:next.feedback?.feedback || '', editorOpen });
}
...
async function handleAction(button) {
  ...
  if (action === 'submit') {
    ...
    try {
      ...
      else if (result.status === 'needs_input') toast('AI 需要补充信息', '请继续回答 AI 提出的关键问题。', 'info');
