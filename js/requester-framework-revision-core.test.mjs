...
assert.equal(selectRequesterFlowState({ task:{status:'reviewing'}, template:{id:'t1'}, revisions:[], history:[{action:'approve_framework'},{action:'reject_draft',reply:'P2乱码'}] }).kind, 'content_revision_requested');
...
assert.equal(selectRequesterFlowState({ task:{status:'reviewing'}, template:{id:'t1'}, revisions:[{revision_no:1,status:'content_ready',affected_pages:[2,3]}], history:[] }).kind, 'content_revision_waiting_ai');
...
console.log('requester framework revision core: 11/11 passed');
