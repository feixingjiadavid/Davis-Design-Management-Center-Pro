...
async function renderAiRequirementPanel() {
    ...
    try {
        ...
        const chatStatus = globalTaskData.status === 'processing' ? 'AI 正在处理…' : openQuestions.length ? `等待你回答 ${openQuestions.length} 个问题` : analysis?.status === 'understanding_ready' ? '信息已齐全' : '当前无需补充';
        const emptyQuestionsHtml = globalTaskData.status === 'processing'
            ? '<div class="flex items-center gap-2 text-indigo-300"><span class="w-2 h-2 rounded-full bg-indigo-400 animate-pulse"></span>AI 正在结合你的回答重新判断，完成后会立即在这里回复。</div>'
            : '<p class="text-emerald-400">AI 暂时没有需要你回答的问题。</p>';
        content.innerHTML = `<section id="ai-clarification-chat" class="rounded-2xl border border-indigo-500/20 bg-indigo-500/5 p-6"><div class="flex justify-between gap-3 mb-5"><div><h3 class="text-lg font-bold text-white">与 AI 设计师沟通</h3><p class="text-sm text-zinc-400 mt-1">回答 AI 提出的问题，提交后它会立即继续处理。</p></div><span class="text-sm text-indigo-300">${chatStatus}</span></div><div class="max-h-72 overflow-y-auto space-y-3 mb-4">${messagesHtml}</div><div class="space-y-4">${questionsHtml || emptyQuestionsHtml}</div>${openQuestions.length ? '<textarea id="ai-general-message" class="mt-4 w-full min-h-[80px] rounded-xl bg-black/30 border border-white/10 p-3 text-sm text-white outline-none focus:border-indigo-400" placeholder="还可以补充其他说明（选填）"></textarea><div class="mt-3 flex flex-wrap gap-3"><button id="ai-chat-submit" onclick="window.submitAiChatAnswers()" class="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold">提交回答</button><button onclick="window.delegateAiClarifications()" class="px-5 py-2.5 rounded-xl bg-zinc-700 hover:bg-zinc-600 text-white font-bold">不确定的交给 AI 决定</button></div>' : ''}<p id="ai-chat-status" class="mt-3 text-xs text-zinc-500"></p></section>${controls ? `<div class="mt-5 flex flex-wrap gap-3">${controls}</div>` : ''}${demo?.output?.image_url ? `<div class="mt-5"><p class="font-bold text-white mb-2">Demo 版本</p><img src="${escapeAiHtml(demo.output.image_url)}" class="max-h-[520px] rounded-xl border border-white/10"></div>` : ''}${final?.output?.image_url ? `<div class="mt-5"><p class="font-bold text-white mb-2">Seedream 4.0 成品</p><img src="${escapeAiHtml(final.output.image_url)}" class="max-h-[520px] rounded-xl border border-white/10"></div>` : ''}`;
...
window.refreshAiUnderstanding = async function() {
    try {
        ...
        window.showToast('AI 已处理', 'AI 已重新检查需求信息。', 'success');
        await fetchTaskData();
...
function renderSmartActionPanel() {
    ...
    if (workflowState.kind === 'needs_input') {
        ...
        pnlHtml = `<div class="absolute right-0 top-0 bottom-0 w-1.5 bg-amber-500"></div><h3 class="text-[16px] font-bold text-amber-400 mb-3">AI 设计师需要您补充需求信息</h3><p class="text-[12px] text-zinc-300 leading-relaxed mb-6">请在下方沟通窗口直接回答 AI 的问题。提交后 AI 会立即继续处理。</p><button onclick="document.getElementById('ai-requirement-panel')?.scrollIntoView({behavior:'smooth',block:'start'})" class="w-full py-3.5 bg-amber-500 hover:bg-amber-400 text-black rounded-xl text-[13px] font-bold">前往回答 AI 问题</button>`;
        ...
        pnlHtml = `<h3 class="text-[16px] font-bold text-indigo-400 mb-3">AI 已拿到完整信息</h3><p class="text-[12px] text-zinc-400 leading-relaxed">现在可以进入 Demo 出图阶段。</p>`;
