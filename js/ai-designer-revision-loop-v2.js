...
async function load(taskId) {
  const [taskResult, templateResult, revisionsResult, analysesResult] = await Promise.all([
  ...
  return { task, template, revisions, latest, history, feedback, analyses, analysis };
}
...
function revisionTimeline(revisions) {
  ...
  if (!items.length) return '<p class="text-xs text-slate-500">尚未产生内容修改轮次。</p>';
  return `<div class="space-y-3">${items.map((revision) => {
    const stage = revisionStage(revision.status);
    const feedback = String(revision.system_content || revision.change_summary?.requester_feedback || '').trim();
...
function analysisHtml(context) {
  ...
  if (!revision || !analysis) return '<div class="rounded-xl border border-blue-500/20 bg-blue-950/15 p-5"><p class="text-sm font-bold text-blue-200">AI 正在进行真实需求理解</p><p class="text-xs text-slate-400 mt-2">会调用当前需求理解服务，判断业务变化、设计稿纠错和受影响页面，不是占位状态。</p></div>';
  ...
  const pageRows = (Array.isArray(brief.pages) ? brief.pages : []).filter((page) => (revision.affected_pages || []).map(Number).includes(Number(page.index || page.page_index))).map((page) => `<li class="text-xs text-slate-300">P${Number(page.index || page.page_index)} ${esc(page.title || '')}${page.copy ? `：${esc(String(page.copy).slice(0,140))}` : ''}</li>`).join('');
  return `<div class="rounded-xl border border-blue-500/20 bg-blue-950/15 p-5"><div class="flex items-center justify-between gap-3"><p class="text-sm font-bold text-blue-200">AI 真实理解结果 · 第 ${revision.revision_no} 次修改</p><span class="text-[11px] text-blue-300">analysis v${analysis.version || '-'}${confidence ? ` · ${Math.round(confidence*100)}%` : ''}</span></div><p class="text-xs text-slate-400 mt-3">判断受影响页面：<span class="text-white font-bold">${esc(pages)}</span></p>${reason?`<p class="text-xs text-slate-400 mt-2">判断原因：${esc(reason)}</p>`:''}${pageRows?`<ul class="mt-3 space-y-2">${pageRows}</ul>`:''}</div>`;
}

function currentStatus(context) {
  const { latest, feedback, task } = context;
...
async function processUncoveredFeedback(context) {
  ...
  if (['completed','archived'].includes(String(context.task.status || '')) || busy) return;
...
async function sync(force = false) {
  ...
  try {
    ...
    console.error('AI revision loop v2 同步失败:', error);
  }
}
